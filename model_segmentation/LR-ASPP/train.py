...
# 4.构架DeepLab+BackBone的模型架构
def create_model(num_classes, pretrain=True):
    # 如果pretrain=False, 则不使用lr-aspp模型的权重
    model = lraspp_mobilenetv3_large(num_classes=num_classes)
    # 可以只载入backbone的权重
    # model = lraspp_mobilenetv3_large(num_classes=num_classes, pretrain_backbone=True)
    if pretrain:
        weights_dict = torch.load("./pre/lraspp_mobilenet_v3_large.pth", map_location='cpu')
        # 重构全连接层
        if num_classes != 21:
            # 官方提供的预训练权重是21类(包括背景)
...
def main(args):
    ...
    # 3.4创建学习率更新策略，这里是每个step更新一次(不是每个epoch)
