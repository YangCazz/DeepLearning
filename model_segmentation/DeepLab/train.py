...
from src import deeplabv3_resnet50, deeplabv3_resnet101, deeplabv3_mobilenetv3_large
...
def create_model(aux, num_classes, pretrain=True):
    ...
    # 使用预训练权重进行训练
    if pretrain:
        weights_dict = torch.load("./pre/deeplabv3_resnet50_coco.pth", map_location='cpu')
        # 重构全连接层
        if num_classes != 21:
            # 官方提供的预训练权重是21类(包括背景)
