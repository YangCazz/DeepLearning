...
def main(args):
    # 0.检查系统中GPU的状态
    try:
        print('系统中可使用的GPU数量:{}'.format(torch.cuda.device_count()))
        print('GPU的名称为:{}'.format(torch.cuda.get_device_name(0)))
    except:
        pass
    ...
    # 创建存储路径
    path = './multi_GPU/weight' + '/' + 'huawei' + '/' + 'ConvNeXt_' + args.model_type
    ...
    if args.model_type == 'tiny':
        model = convnext_tiny(num_classes=args.num_classes)
        ...
        model = convnext_large(num_classes=args.num_classes)
    elif args.model_type == 'xlarge':
        model = convnext_xlarge(num_classes=args.num_classes)
    ...
    if args.weights != "":
        assert os.path.exists(args.weights), "预训练权重: '{}' 不存在.".format(args.weights)
        weights_dict = torch.load(args.weights, map_location=device_ids[0])["model"]
        # 删除有关分类类别的权重
